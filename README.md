...
The following repository was cloned from d'Ascoli's article "[Towards decoding individual words from non-invasive brain recordings](https://www.nature.com/articles/s41467-025-65499-0)". The repo contains experiments done by team Columbia for the [NeuroHack 2026](https://global-neurohack.github.io) in San Francisco. In addition to the baseline implementation, we included our own collected EEG work station, where we collected brain signals from one of our own team members using the BCI Core-8, which is a consumer-grade, 8-channel headset sampling at 250Hz. In terms of the pipeline, we experimented with using N400 and theta band signals to track semantic understanding and how those signals can aid the transformer in guessing the next spoken word. We also added an LLM at the end of the pipeline to help modify the sentence into a logical one.
